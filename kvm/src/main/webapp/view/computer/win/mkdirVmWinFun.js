	...
	HyAjax.Get("/kvmServer/doPageSelect.action",function (res){
		...
		if(data.length > 0){
		    ...
		    HyAjax.Get("/kvmServer/getIsoAndDirverAndNetwork.action?kvmServerId=" + data[0].kvmServerId , function(result) {
		        if (result.success) {
					if(result.hasOwnProperty("bootServers")){
						bootServers = result.bootServers;
					}
					...
					}else if(serverType == "aarch"){
						$("#kvmServerType").val("aarch64");
						$("#vmType").html('<option value="aarch">UEFI</option>');
					}else if(serverType == "loongarch64"){
						$("#kvmServerType").val("loongarch64");
						$("#vmType").html('<option value="loongarch">UEFI</option>');
					}
		        	var networkHtml = "";
		        	if(isos != ""){
...
function initEdit(vm){
	$("#vmTypeDiv").attr("disabled","disabled")
	init(vm);
	form.val("vmfilter", {
		  "vmName": vm.vmName
		  ,"vmId": vm.vmId
		...
		, "vmDiskMode":vm.vmDiskMode
		});
	$("#vmTypeInput").html('<input type="text" name="vmType" id="vmType" value="'+vm.vmType+'" disabled="disabled" class="layui-input">');
