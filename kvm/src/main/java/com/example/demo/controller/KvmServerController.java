...
public class KvmServerController {
	...
	public JSONObject doInsert(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception{
		...
		logService.insert(request, "添加服务器("+kvmServer.getKvmServerIp()+")", json.getBoolean("success")?1:0);
		return json;
	}
	...
	public JSONObject powerOff(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception {
		...
		logService.insert(request, "服务器("+kvmServer.getKvmServerIp()+")关机", json.getBoolean("success")?1:0);
		return json;
	}
	...
	public JSONObject reboot(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception {
		...
		logService.insert(request, "服务器("+kvmServer.getKvmServerIp()+")重启", json.getBoolean("success")?1:0);
		return json;
	}
	...
	public JSONObject refresh(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception {
		...
		logService.insert(request, "刷新服务器("+kvmServer.getKvmServerIp()+")状态", json.getBoolean("success")?1:0);
		return json;
	}
	...
	public JSONObject doDelete(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception {
		Integer kvmServerId = condition.getInteger("kvmServerId");
		KvmServer kvmServer = kvmServerService.queryKvmServerById(kvmServerId);
		JSONObject json = kvmServerService.delete(kvmServerId);
		logService.insert(request, "删除服务器("+kvmServer.getKvmServerIp()+")", json.getBoolean("success")?1:0);
		return json;
	}
	...
	public JSONObject deleteKvmImage(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) {
		...
		logService.insert(request, "删除镜像("+imageName+")", json.getBoolean("success")?1:0);
		return json;
	}
	...
	public JSONObject importLicense(@RequestParam(value = "file", required = false) MultipartFile file,@RequestAttribute("condition") CommonVO condition,HttpServletRequest request) throws IllegalStateException, IOException{
		...
		if (!file.isEmpty()) {
			...
			logService.insert(request, "上传镜像("+name+")", json.getBoolean("success")?1:0);
		}
		return json;
}
	...
	public JSONObject uploadDriver(@RequestParam(value = "file", required = false) MultipartFile file,@RequestAttribute("condition") CommonVO condition,HttpServletRequest request) throws IllegalStateException, IOException{
		...
		if (!file.isEmpty()) {
			...
			file.transferTo(destFile);  
			logService.insert(request, "上传驱动("+name+")", json.getBoolean("success")?1:0);
		}
		return json;
	}
	...
	public JSONObject getServerInfo(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception {
		...
		try {
			...
			//获取服务器存在天数
			String createTime = SshUtil.executeLocalRerurn("ls -ld --time-style '+%Y/%m/%d' /var/log/installer | awk '{print $6}'");
			Date now = new Date();
	 ...
	 */
	@ResponseBody
	@RequestMapping(path = "/openServer.action", method = RequestMethod.POST)
	public JSONObject openServer(@RequestAttribute("condition") CommonVO condition, HttpServletRequest request) throws Exception {
		JSONObject json = new JSONObject();
		json.put("success", true);
		KvmServer kvmServer = kvmServerService.queryKvmServerById(condition.getInteger("kvmServerId"));
		String shPath = "/opt/kvm/sh/ttyd.sh";
		String command = "sudo chmod 777 " + shPath + ";sudo nohup sh "+shPath+" > /dev/null  2>&1 &";
		SshUtil.execute(kvmServer, command);
		json.put("url", "http://"+kvmServer.getKvmServerIp()+":14377");
		logService.insert(request, "查看服务器终端", json.getBoolean("success")?1:0);
		return json;
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(path = "/downloadKvmImage.action",method = RequestMethod.POST)
	public JSONObject downloadKvmImage(@RequestAttribute("condition") CommonVO condition,HttpServletRequest request) throws Exception{
		JSONObject json = new JSONObject();
		json.put("success", true);
		KvmServer kvmServer = kvmServerService.getAllKvmServer().get(0);
		String imagePath = condition.getString("imagePath");
		String imageName = condition.getString("imageName");
		String command = "";
		if(imagePath.contains("isos")) {
			command += "cd /home/isos;";
		}else {
			command += "cd /home/drivers;";
		}
		command += "kill -9 $(lsof -i:8000 -t);nohup python3  -m http.server >/dev/null 2>&1 &";
		SshUtil.execute(kvmServer, command);
		json.put("url", "http://"+kvmServer.getKvmServerIp()+":8000/"+imageName);
		logService.insert(request, "下载镜像("+imageName+")", json.getBoolean("success")?1:0);
		return json;
	}
}
