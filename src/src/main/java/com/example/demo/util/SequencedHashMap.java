 ...
 * use {@link Collections#synchronizedMap(Map)} as it is documented, or use
...
public class SequencedHashMap implements Map, Cloneable, Externalizable {
	...
	private static class Entry implements Map.Entry {
		...
		// implementated, then what happens when the linked list is cyclical (as
		// is
	...
	private class OrderedIterator implements Iterator {
		 ...
		 * should be either {@link #KEY}, {@link #VALUE}, or {@link #ENTRY}.
		 * To save a tiny bit of memory, this field is also used as a marker for
		 * when remove has been called on the current object to prevent a second
		 * remove on the same element. Essientially, if this value is negative
		 * (i.e. the bit specified by {@link #REMOVED_MASK} is set), the current
		 ...
		 * specified by <code>returnType</code> which must be either
		 * {@link #KEY}, {@link #VALUE}, or {@link #ENTRY}.
		 */
		 ...
		 *                if there isn't a "last element" to be removed. That
		 *                is, if {@link #next()} has never been called, or if
