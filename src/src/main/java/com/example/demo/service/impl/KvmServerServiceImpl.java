				...
				try {
					...
					for(int id : online){
						...
						for(String i:isos.split("\n")){
							if(i.contains("sda")){
								hda=i.replace(" ", "").replace("sda", "");
							}
							if(i.contains("sdb")){
								hdc=i.replace(" ", "").replace("sdb", "");
							}
						}
						vm.setVmIso(hda.equals("")?"0":hda.substring(11));
						vm.setVmDriver(hdc.equals("")?"0":hdc.substring(14));
						String diskinfo=SshUtil.executeReturn(kvmServer, "sudo virsh domblkinfo "+domain.getName()+" vda");
						if(!diskinfo.equals("\n")) {
							try {
