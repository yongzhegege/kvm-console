...
public class VmServiceImpl implements VmService {
	...
	public JSONObject createVm(CommonVO condition) {
		...
		Integer vmDiskMode = condition.getInteger("vmDiskMode");
		String xmlPath = "";
		if(vmType.equals("legacy_deb")) {
			...
			vmType = "UEFI";
		}else if(vmType.equals("aarch")) {
			try {
				xmlPath = "".equals(SshUtil.executeReturn(kvmServer, "ls /usr/libexec/qemu-kvm"))?"/aarch64-uefi-deb.xml":"/aarch64-uefi-rpm.xml";
			} catch (Exception e) {
				e.printStackTrace();
			} 
			vmType = "UEFI";
		}else if(vmType.equals("loongarch")) {
			try {
				xmlPath = "".equals(SshUtil.executeReturn(kvmServer, "ls /usr/libexec/qemu-kvm"))?"/loongarch64-uefi-deb.xml":"/loongarch64-uefi-rpm.xml";
			} catch (Exception e) {
				e.printStackTrace();
			} 
			vmType = "UEFI";
		}
		xmlPath = PropertiesUtil.get("vmXmlPath") + xmlPath;
		...
		//修改权限
		command += "chmod -R 777 /var/lib/libvirt/images/"+vmName;
		...
		try {
					...
					for(Element E:elements2){
					 ...
					 for(Element E:elements2){
						 if(E.element("driver").attribute("type").getValue().equals("qcow2")){
							 ...
							 if(vmDiskMode == 1) {
								 E.element("target").attribute("dev").setValue("vda");
								 E.element("target").attribute("bus").setValue("virtio");
							 }
						 }
					 }
					 ...
					 if(vmDiskMode == 1) {  //判断如果硬盘模式为virtio 则删除controller
						 List<Element> controllers = e.elements("controller");
						 for(Element E:controllers){
							 if(E.attribute("model").getValue().equals("virtio-scsi")) {
								 E.detach();
								 break;
							 }
						 }
			...
			xmlDesc=docu.asXML();
				...
				if(domain!=null){
					//判断是否安装镜像
					if(!vmIso.equals("0")){
						if(xmlPath.contains("aarch64") || xmlPath.contains("loongarch64")) {  //判断sata 还是 scsi
							SshUtil.execute(kvmServer, "sudo virsh attach-disk "+domain.getName()+" /home/isos/"+vmIso+" sde --targetbus scsi --driver qemu --type cdrom --mode readonly --config");
						}else {
							SshUtil.execute(kvmServer, "sudo virsh attach-disk "+domain.getName()+" /home/isos/"+vmIso+" sde --targetbus sata --driver qemu --type cdrom --mode readonly --config");
						}
					}
					//判断是否安装驱动
					if(!vmDriver.equals("0")){
						if(xmlPath.contains("aarch64") || xmlPath.contains("loongarch64")) {  //判断sata 还是 scsi
							SshUtil.execute(kvmServer, "sudo virsh attach-disk "+domain.getName()+" /home/drivers/"+vmDriver+" sdf --targetbus scsi --driver qemu --type cdrom --mode readonly --config");
						}else {
							SshUtil.execute(kvmServer, "sudo virsh attach-disk "+domain.getName()+" /home/drivers/"+vmDriver+" sdf --targetbus sata --driver qemu --type cdrom --mode readonly --config");
						}
					}
					//写入token
					...
					//写入VM表
					Vm vm = new Vm(vmName, vmName, "", "", mac, 0, kvmServerId, vncPort, vmIso, vmDriver, vmMemory, vmDisk, vmCpu, vmNetwork, vmUuid,vmDefaultStart,vmType,vmDiskMode);
					vmDao.insert(vm);
					//更改kvmServer表vmNum
	...
	public JSONObject editVm(CommonVO condition) {
		...
		String serverType = "";
		try {
			 String uname = SshUtil.executeReturn(kvmServer, "sudo uname -m");
			 if(uname.contains("x86_64")) {
				serverType = "sata";
			 }else if(uname.contains("aarch64") || uname.contains("loongarch64")) {
				 serverType  = "scsi";
			 }
		} catch (Exception e) {}
		//修改内存
		...
		try {
			//修改硬盘模式
			if(vmDiskMode != oldVM.getVmDiskMode()) {
				if(oldVM.getVmDiskMode() == 0) {
					command += "sed -i \"s/<target dev='sda' bus='scsi'/<target dev='vda' bus='virtio'/\" /etc/libvirt/qemu/" + oldVM.getVmName() + ".xml;";
					command += "sed -i '/vda/{n;d}' /etc/libvirt/qemu/"+oldVM.getVmName() + ".xml;";
					command += "sed -i '/virtio-scsi/,+2d' /etc/libvirt/qemu/"+oldVM.getVmName() + ".xml;";
				}else {
					command += "sed -i \"s/<target dev='vda' bus='virtio'/<target dev='sda' bus='scsi'/\" /etc/libvirt/qemu/" + oldVM.getVmName() + ".xml;";
					command += "sed -i '/sda/{n;d}' /etc/libvirt/qemu/"+oldVM.getVmName() + ".xml;";
					command += "sed -i \"/<emulator>/a\\<controller type='scsi' index='0' model='virtio-scsi'/>\" /etc/libvirt/qemu/"+oldVM.getVmName() + ".xml;";
				}
				oldVM.setVmDiskMode(vmDiskMode);
			}
			//修改ISO
			if(!oldVM.getVmIso().equals(vmIso)){
				if(oldVM.getVmIso().equals("0")){
					command1 += "sudo virsh attach-disk "+oldVM.getVmName()+" /home/isos/"+vmIso+" sde --targetbus "+serverType+" --driver qemu --type cdrom --mode readonly --config;";
				}
				if(vmIso.equals("0")){
					command1 += "sudo virsh  detach-disk "+oldVM.getVmName()+" sde --config;";
				}
				if(!oldVM.getVmIso().equals("0") && !vmIso.equals("0")){
					command1 += "sudo virsh  detach-disk "+oldVM.getVmName()+" sde --config;sudo virsh attach-disk "+oldVM.getVmName()+" /home/isos/"+vmIso+" sde --targetbus "+serverType+" --driver qemu --type cdrom --mode readonly --config;";
				}
			...
			if(!oldVM.getVmDriver().equals(vmDriver)){
				if(oldVM.getVmDriver().equals("0")){
					command1 += "sudo virsh attach-disk " + oldVM.getVmName() + " /home/drivers/" + vmDriver + " sdf --targetbus "+serverType+" --driver qemu --type cdrom --mode readonly --config;";
				}
				if(vmDriver.equals("0")){
					command1 += "sudo virsh  detach-disk "+oldVM.getVmName()+" sdf --config;";
				}
				if(!oldVM.getVmDriver().equals("0") && !vmDriver.equals("0")){
					command1 += "sudo virsh  detach-disk "+oldVM.getVmName()+" sdf --config;sudo virsh attach-disk " + oldVM.getVmName() + " /home/drivers/" + vmDriver + " sdf --targetbus "+serverType+" --driver qemu --type cdrom --mode readonly --config;";
				}
