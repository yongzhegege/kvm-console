...
function bindUser(){
	...
	if (checkVm.length !=1) {
		...
		layui.layer.open({
			...
			title: '绑定用户',
			area: ['800px', '550px'],
			content: ['win/bindUser.html', 'no'],
			success: function(layero, index) {
				var iframeWin = window[layero.find('iframe')[0]['name']];
//				iframeWin.vmInit(vmIds.substring(1))
			}
		})
	}
}
